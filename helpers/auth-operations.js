...
export const registerUserHelper = async (userData) => {
  try {
    ...
    if (profilePicture) {
      ...
      const normalizedProfilePicture = profilePicture.replace(/\\/g, '/');
      const normalizedUploadPath = uploadPath
        ? uploadPath.replace(/\\/g, '/')
        : '';

      const isLocalFile =
        normalizedProfilePicture.includes('uploads/') ||
        (normalizedUploadPath &&
          normalizedProfilePicture.includes(normalizedUploadPath)) ||
        normalizedProfilePicture.startsWith('./');

      if (isLocalFile) {
        try {
          const ext = path.extname(normalizedProfilePicture);
        ...
        try {
          ...
          let normalized = normalizedProfilePicture;
